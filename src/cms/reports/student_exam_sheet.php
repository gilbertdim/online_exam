...
$cn->query("SELECT * FROM vw_exam_examinees WHERE exam_id = $examid AND exam_code = '$examcode' AND studentno = '$studentno'");
...
$exam_total = $row['totalscore'];
$exam_start = date_format(date_create($row['examstart']), 'M d, Y h:i A'); 
$exam_end = date_format(date_create($row['examend']), 'M d, Y h:i A');
...
$file_name = "Exam $student_name $exam_subj $exam_title";
